...
Inputs: notebook entry from `opt_config` (`source_tables`, `target_tables`, `operation`, `sandbox_catalog`).
...
2. **Clone targets**: `CREATE TABLE <sandbox_catalog>.<schema>.<t> SHALLOW CLONE <prod>` for each
   `target_table`. Shallow clone is zero-copy and carries current data — required so a MERGE
   behaves exactly like prod. (Empty target => wrong merge result.)
3. **Remap writes**: rewrite the v2 notebook's write targets to `sandbox_catalog` (same
   schema.table, different catalog). Verify no write points at a production table.
